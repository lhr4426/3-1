    ...
    for row in range(1, h - 1):
        for col in range(1, w - 1):
            ...
            if 0 <= degree and degree < 45:
                ...
                rate = np.tan(np.deg2rad(degree - 45))
                ...
                rate = np.tan(np.deg2rad(degree + 90))
    ...
    for row in range(h):
        for col in range(w):
            if dst[row, col] >= high_threshold_value:
                ...
                #######################################
                max_item = np.max(dst[row -1:row+2, col - 1:col + 2])
                if max_item != dst[row, col] and (max_item >= high_threshold_value):
                    dst[row, col] = 255
                
    for first_row in range(h) :
        for first_col in range(w) :
            if dst[first_row, first_col] >= low_threshold_value and dst[first_row, first_col] < high_threshold_value :
                check_list = [[first_row, first_col]]
                next_row = first_row
                next_col = first_col
                flag = True
                while (flag) :
                    for row in range(next_row-1,next_row+2) :
                        for col in range(next_col-1,next_col+2) :
                            if row != next_row and col != next_col and dst[row, col] >= low_threshold_value and dst[row, col] < high_threshold_value :
                                check_list.append([row, col])
                                next_row = row
                                next_col = col
                            elif row != next_row and col != next_col and dst[row, col] >= high_threshold_value :
                                check_list.append([row, col])
...
def my_canny_edge_detection(src, fsize=3, sigma=1):
    """
    한글이나 영어로 작성하기
    ID, name(학번 이름): 20211924, Lee Hye Rim
    ...
    cv2.imshow('magnitude - 20211924 Lee Hye Rim', convert_uint8(magnitude))

    angle = calcAngle(Ix, Iy)

    # non-maximum suppression 수행
    larger_magnitude = non_maximum_supression(magnitude, angle)
    cv2.imshow('NMS - 20211924 Lee Hye Rim', convert_uint8(larger_magnitude))

    # strong / weak edge 확인용
    strong_edge_img, weak_edge_img = show_strong_weak_edge(larger_magnitude)
    cv2.imshow('strong edge - 20211924 Lee Hye Rim', strong_edge_img)
    cv2.imshow('weak edge - 20211924 Lee Hye Rim', weak_edge_img)
...
def main():
    src = cv2.imread('ImageProcessing/week6/homework/Lena.png', cv2.IMREAD_GRAYSCALE)

    dst = my_canny_edge_detection(src)

    cv2.imshow('original - 20211924 Lee Hye Rim', src)
    cv2.imshow('my canny edge detection - 20211924 Lee Hye Rim', dst)
    cv2.waitKey()
