...
def my_bilateral(src, msize, sigma, sigma_r, pos_x, pos_y, pad_type='zero'):
    ...
    y, x = np.mgrid[-(m_s[0]//2):(m_s[0]//2)+1, -(m_s[1]//2):(m_s[1]//2)+1]
    mask_gaus = np.exp(-(x**2)/((2*sigma)**2)-(y**2)/((2*sigma)**2))

    for i in range(h):
        print('\r%d / %d ...' %(i,h), end="")
        for j in range(w):
            img_mid = np.full(m_s, src[i,j])
            img_crop = img_pad[i:i+m_s[0], j:j+m_s[1]]
            mask = mask_gaus * np.exp(-((img_mid - img_crop)**2)/(2*sigma_r)**2)
            mask /= np.sum(mask)
            val = np.sum(img_crop * mask)
...
if __name__ == '__main__':
    start = time.time()
    src = cv2.imread('ImageProcessing/week9/homework/Lena.png', cv2.IMREAD_GRAYSCALE)
    ...
    ######################################################
    dst = my_bilateral(src_noise, 5, 3, 5, pos_x, pos_y)
